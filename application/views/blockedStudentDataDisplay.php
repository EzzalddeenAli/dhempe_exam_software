...
<div class="content">
	 <div class="gadget">
		...
		<?php echo form_open('home/saveBlockedDetails',$form_id);
		$js ='class="my_select"';
		...
		?>
			<?php if(!empty($student_details)){ ?>
				<table class="main_table" border=1 width=100%;> <thead><tr >
					...
					<th class="table-header-repeat line-left minwidth-2">Blocked and Now Eligible</th>
					...
					if($row['supplementary'] == 1)
						$type="Supplementary";
					if($row['now_eligible'] == 1)
						$nowEligibleChecked=" checked " ;
					?>				
					<tr>
						...
						<td><?php echo $type; ?></td>
						<td><?php echo '<input type="checkbox"  name="blocked_'.$i.'" checked onclick="checkBlock(this,'.$i.');"/>'; ?></td>
						<td><?php  echo '<input type="checkbox"  name="now_eligible_'.$i.'" id="now_eligible_'.$i.'" '.$nowEligibleChecked.' />'; ?></td>
