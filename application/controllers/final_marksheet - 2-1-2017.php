<?php
	...
	class final_marksheet extends CI_Controller{
		...
		function now_eligible_print_marksheet(){
		...
		function nowEligibleChecklist(){
			$data['type']='now_eligible';
			$this->load->view('checklist',$data);
		}
		
		/*modified code on 29 april 2013 starts here*/
		function supple_check_list(){
			$data['type']='all';
			$this->load->view('supple_checklist',$data);
		}function nowEligibleChecklistSupplementary(){
			$data['type']='now_eligible';
			$this->load->view('supple_checklist',$data);
		}
		...
		function semester_checklist($typeOfGeneration=''){
			...
			* Before generating check list checking all student data is entered or not.
			*
			*/
			...
			if($pr_counter == 0){
				die("Note :- No Student Found");
			}
			if(($tempSem == 1)||($tempSem == 2)){
				$soft_assumption = $pr_counter*8; //Assuming this much row should be present in enteres marks table
				
				//check all marks are available 
				$tbl_counter = $this->dataentry->getCountofRows($stream_tbl_name);
				
				if($tbl_counter != $soft_assumption)
					die("Note :- Few Student Marks Have not been Entered ");
			}			
			else if(($tempSem == 3)||($tempSem == 4)){
				$soft_assumption = $pr_counter*7; //Assuming this much row should be present in enteres marks table
			...
			//check marks are calculated or not before generating checklist
			$pass_flag = $this->dataentry->checkMarksStatus($stream_tbl_name);
			if($pass_flag == 0){
				die("Note:- Calculate Student Marks First");
			}*/
...
$mpdf->WriteHTML('<div style="text-align: center; font-size:18px; padding-bottom:20px;">Stream : '.$folder.' Semester : '.$tempSem.'</div>');
	...
	//**************************************************************sachin9****************************//	
		...
		function gen_mar_2012($stream,$semester,$number,$option)
		{
			require_once("Mpdf/Mpdf.php");
			require_once('numtowords1.php');
		...
		function generate_marksheet_new($typeOfGeneration=''){
			$year=$this->input->post('year');
			...
			/******************PDF**********************/
				...
				foreach($pr_marks as $row2){
					...
					*/
					...
					if($paramSet == 0){
					// "sdnasldnasd".$row4->practical_marks;die();
						if($row4->practical_marks != -1){	
							$practTemp=$row4->practical_marks;
							$mpdf->WriteHTML('
								<tr>
					...
					//entitlement grace
						...
						//general grace
							...
							//$finalcal+=$row2->total;
							/*Changed by simone since total not correct*/
							$finalcal+=$row2->internal + $row2->theory;
						
						/*not sure if required--> giving wrong result of E/N*/
						/* if(($paramSet == 1)&&($row2->practicle != -1)){
						//echo "iam here";die();
							if($row4->practical_marks != -1){	
								if($paramSet == 1){
								//echo '<br /> isa ='.$isa.' see='.$see.' temp='.$temp_result2.' minimum_theory='.$row4->minimum_theory;
									//commented by simo ne since no hardcoding
									//if(($isa+$see+$temp_result2+$temp_result >= 30)&&($flag_seted != 1)){
									if(($isa+$see+$temp_result2+$temp_result >= $row4->minimum_theory)&&($flag_seted != 1)){
										$print_tag='E';
									}else if($flag_seted != 1){
										$print_tag='N';
									}
						...
						$sub_total=$row2->internal + $row2->theory;
							...
							<tr>
								...
								<td>'.($row4->internal_marks+$row4->semester_marks+$practTemp).'</td>
								<td>'.(($row4->internal_marks+$row4->semester_marks+$practTemp)*40/100).'</td>
								<td>'.$sub_total.' '.$display_theory_grace.'</td>
							</tr>');
						}
						...
						if(($paramSet == 1)&&($row2->practicle != -1)){
							if($row4->practical_marks != -1){	
								...
								if( $row2->pract_abs ==""){ $pract=$row2->practicle; } else { $pract="ABS";}
								/*commented by simone since max/min marks cant be hardcoded*/
								$mpdf->WriteHTML('
									<tr>
										<td></td>
										<td></td>
										...
										<td>'.$row4->practical_marks.'</td>
									<td>'.$row4->min_practical.'</td>
										<td>'.$pract. $row2->gen_the_pract_sym.'  '.$ptag.'</td>
									</tr>
								');
								$finalcal+=$row2->practicle;
							}
		...
		function bulk_dataentry(){
			$this->load->view('bulkData');	
		}
		
		function bulk_dataentry2(){
			$this->load->view('bulkData2');	
		}
		
		function bulk_dataDisplay(){
			require_once("Mpdf/Mpdf.php");
			//require_once('numtowords1.php');
		...
		/* Modified on 24 april 2013 ends*/
		
		/*modified code on 29 april 2013 starts
			function supple_semester_checklist(){
			require_once("Mpdf/Mpdf.php");
...
$mpdf->WriteHTML('<div style="text-align: center; font-size:18px; padding-bottom:20px;">Stream : '.$folder.' Semester : '.$tempSem.'</div>');
		...
		*/
			function supple_semester_checklist($typeOfGeneration=''){
			require_once("Mpdf/Mpdf.php");
...
$mpdf->WriteHTML('<div style="text-align: center; font-size:18px; padding-bottom:20px;">Stream : '.$folder.' Semester : '.$tempSem.'</div>');
		...
		function bulk_dataentry4(){
			$this->load->view('bulkData4');	
		}
		
		function bulk_dataDisplay4(){
			require_once("Mpdf/Mpdf.php");
			//require_once('numtowords1.php');
